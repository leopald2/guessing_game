...
def find_best_choice(state, future_states):
	actions_state = future_states.loc[[state]]
...
def update_q_values(states_map, future_states ,state, choice, new_state, alpha, gamma, neg_r):
	if(new_state != 'won'):
		...
		actions_new_state = future_new_states.loc[[new_state]]
		...
		states_map.loc[[state], choice] += alpha*(neg_r + gamma*max_q_new_state - states_map.loc[[state], choice])
